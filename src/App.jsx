...
function App() {
  ...
  if (!isAuthenticated) {
    return authView === "login" ? <Login onSwitchToRegister={() => setAuthView("register")} /> : <Register onSwitchToLogin={() => setAuthView("login")} />;
  }
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case "read":
        return <QuranReader params={viewParams} onBack={() => handleNavigate("dashboard")} />;
      case "surahList":
        return <SurahListPage onNavigate={handleNavigate} onBack={() => handleNavigate("dashboard")} />;
      case "log":
        return <Log />;
      case "settings":
        return <Settings />;
  ...
  const getPageVariants = (view) => {
    if (view === "read") {
      // Changed to 'read' to match currentView value
      return {
        ...
        <AnimatePresence mode="wait" initial={false}>
          <Layout currentView={currentView} onNavigate={handleNavigate}>
            <AnimatePresence mode="wait">
              <motion.div key={currentView} initial={pageVariants.initial} animate={pageVariants.animate} exit={pageVariants.exit} transition={pageVariants.transition} className="h-full">
                {renderView()}
              </motion.div>
            </AnimatePresence>
          </Layout>
